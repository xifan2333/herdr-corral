...
impl Config {
    ...
    pub fn load() -> Self {
        ...
        // Existing user config is authoritative and is never rewritten.
        // If seeding failed (for example on a read-only config directory), use
        // the shipped default in memory without touching the user's filesystem.
